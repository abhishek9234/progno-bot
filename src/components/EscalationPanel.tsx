    ...
    >
      <div className="space-y-4">
        ...
        <div className={`p-4 rounded-lg border ${
          ...
          : metrics.level <= 1 ? 'bg-warning/10 border-warning/30'
          : 'bg-destructive/10 border-destructive/30'
        }`}>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <ArrowUpCircle className={`h-5 w-5 ${
                metrics.level === 0 ? 'text-success' 
                : metrics.level <= 1 ? 'text-warning'
                : 'text-destructive'
              }`} />
              <span className="font-medium">Current Level</span>
            </div>
            <Badge variant="outline" className={`font-mono ${
              metrics.level === 0 ? 'bg-success/10 text-success border-success/30' 
              : metrics.level <= 1 ? 'bg-warning/10 text-warning border-warning/30'
            ...
            {[0, 1, 2, 3].map((level) => (
              <div 
                ...
                className={`h-2 flex-1 rounded-full ${
                  level < metrics.level 
                    ? level < 2 ? 'bg-warning' : 'bg-destructive'
                    : 'bg-secondary'
                }`}
              />
            ))}
